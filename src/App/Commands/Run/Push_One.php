...
/**
 * Handler for run push-one command
 *
 * @author DC
 */
class Push_One implements Command_Handler_Interface
{
    /**
     * flags
     *
    ...
    public function handle(): void
    {
        ...
        if ($version_file === 'error') {
            Output::error("This file cannot be run because its last run resulted in an error! $file");
        ...
        if ($version_file !== false) {
            Output::error("This file has already been ran! $file");
            $this->trigger('script-end');
            return;
        }
